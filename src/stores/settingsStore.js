...
const loadSettings = () => {
  ...
  try {
    ...
    // Older builds called settings.update() with no arguments to mean "save",
    // which spread an undefined key into the object and persisted it. Harmless,
    // but it should not keep being carried forward.
...
const createSettingsStore = () => {
  ...
  return {
    ...
    update: (key, value) => {
      update(current => {
        // Called with no arguments to mean "a nested field was mutated in
        // place, persist and notify". Spreading an undefined key wrote a
        // literal `undefined` entry into the saved settings every time.
        const updated = key === undefined ? { ...current } : { ...current, [key]: value }
