<script>
  ...
  // Constant Change can draw a rotation speed of 0, and the settings slider
  // goes down to 0 too. Dividing by it yields Infinity, which is not a valid
  // animation-duration - the browser drops the declaration and the cube keeps
  // spinning at whatever it was doing before. Treat 0 as "hold still".
  $: isSpinning = Number($settings.rotationSpeed) > 0
  $: rotationTime = isSpinning ? (3400 / $settings.rotationSpeed).toFixed(0) : 1
  ...
  $: maxWidth = $gameDisplayInfo ? gameDisplayInfo.getMaxWidth() : 1
  $: highlight = presentation.highlight
  $: flash = presentation.flash
  // Read the running game's grid, not the live setting. Without the store
  // prefix this was reading a property off the store object itself, which is
  // always undefined, so the board silently followed whatever the settings
  // said - including a mid-game change written by the Constant Change sync.
