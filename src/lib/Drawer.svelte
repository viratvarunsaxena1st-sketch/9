...
const getPositionWidthDisplay = () => {
  // `!mode === 'tally'` compared a boolean against a string, so this guard
  // never fired. It is harmless today because the caller is already inside a
  // tally branch, but it would misread the moment anything else called it.
